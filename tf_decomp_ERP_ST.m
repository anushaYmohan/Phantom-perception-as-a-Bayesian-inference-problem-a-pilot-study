function [tf_db, as_final, as_freq] = tf_decomp_ERP_ST(data, Channels, Times_new)
...
size_data = size(data);
EEG.trials = size_data(1);
EEG.data = reshape(data, length(Channels), length(Times_new), EEG.trials);
...
for elec = 1 : length(Channels)
    ...
    dataX = fft(reshape(EEG.data(elec,:,:), 1,[]),nConv);
